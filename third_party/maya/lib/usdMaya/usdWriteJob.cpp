...
usdWriteJob::usdWriteJob(const JobExportArgs & iArgs) :
    mArgs(iArgs), mModelKindWriter(iArgs), mRefEditUtil(iArgs.mergeTransformAndShape)
{
}
        ...
        MDagPath &curDag, MayaPrimWriterPtr* primWriterOut)
{
    ...
    if (ob.hasFn(MFn::kPluginDependNode) && ob.hasFn(MFn::kDagNode) && ob.hasFn(MFn::kDependencyNode)) {
        ...
        if (PxrUsdMayaPrimWriterRegistry::WriterFn primWriter =
            ...
            PxrUsdExport_PluginPrimWriter::Ptr primPtr(new PxrUsdExport_PluginPrimWriter(
                        curDag, mStage, mArgs, refEdits, primWriter));
            if (primPtr->isValid()) {
    ...
    if (ob.hasFn(MFn::kTransform) || ob.hasFn(MFn::kLocator)) {
        MayaTransformWriterPtr primPtr(new MayaTransformWriter(curDag, mStage, mArgs, refEdits));
        if (primPtr->isValid() ) {
            *primWriterOut = primPtr;
            return true;
        }
        ...
        MayaMeshWriterPtr primPtr(new MayaMeshWriter(curDag, mStage, mArgs, refEdits));
        if (primPtr->isValid() ) {
            *primWriterOut = primPtr;
            return true;
        }
        ...
        MayaNurbsCurveWriterPtr primPtr(new MayaNurbsCurveWriter(curDag, mStage, mArgs, refEdits));
        if (primPtr->isValid() ) {
            *primWriterOut = primPtr;
            return true;
        }
        ...
        MayaNurbsSurfaceWriterPtr primPtr(new MayaNurbsSurfaceWriter(curDag, mStage, mArgs, refEdits));
        if (primPtr->isValid() ) {
            *primWriterOut = primPtr;
            return true;
        }
        ...
        MayaCameraWriterPtr primPtr(new MayaCameraWriter(curDag, mStage, mArgs, refEdits));
        if (primPtr->isValid() ) {
            *primWriterOut = primPtr;
            return true;
        }
