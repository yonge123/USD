...
/// \class GfRGB
///
/// A color represented as 3 floats for red, green, and blue.
///
...
class GfRGB {
  ...
  public:
    ...
    /// Returns \code (1-alpha) * a + alpha * b \endcode similar to GfLerp for
    /// other vector types.
    ...
    /// \note This assumes normalized RGB channels in [0,1] and doesn't work
    /// with HDR color values.
    GfRGB GetComplement() const {
        return GfRGB(1) - *this;
    }

    /// Return the luminance of a color given a set of RGB weighting values.
    /// Defaults are Rec.709 weights for linear RGB components.
    ...
    /// Return the luminance of a color given a set of RGB weighting values
    /// passed as a GfRGB color object.
