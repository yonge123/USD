...
///
///
class UsdHydraUdimTexture : public UsdHydraUvTexture
{
public:
...
public:
    ...
    // UDIMENSION 
    // --------------------------------------------------------------------- //
    /// Specifies the U dimension when generating the texture name.
    ///
    /// \n  C++ Type: int
    /// \n  Usd Type: SdfValueTypeNames->Int
    /// \n  Variability: SdfVariabilityVarying
    /// \n  Fallback Value: 10
    UsdAttribute GetUDimensionAttr() const;

    /// See GetUDimensionAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    UsdAttribute CreateUDimensionAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
